...
// <style id="formbuilder-tokens">:root{ … }</style> block into the frame's
// own <head> from params.tokens (a {"--name":"value"} map). Because this
// <style> is appended AFTER builder.css, it wins on equal :root specificity.
// The frame CSS is token-only with inline fallbacks, so it is legible before
// init.
//
// Light/dark is entirely token-driven here — NO prefers-color-scheme gating
// (the repo rule: theme is host-driven).

const STYLE_ID = "formbuilder-tokens";
