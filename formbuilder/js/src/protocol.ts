...
//
// Structurally identical to the datagrid/mermaid frame client: the protocol
// is frozen and lives in the platform, so every plugin frames the same
// client. Handler params are `unknown` (never `any`) — each handler narrows
// its own payload defensively.
  ...
  return function route(event: MessageEvent): void {
    ...
    if (msg.type === "event") {
      try {
        ...
        console.error("[formbuilder] event handler error:", msg.method, err);
      }
      return;
    }
    if (msg.type === "request") {
