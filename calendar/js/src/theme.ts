...
// <style id="calendar-tokens">:root{ … }</style> block into the frame's own
// <head> from params.tokens (a {"--name":"value"} map). Because this <style>
// is appended AFTER app.css, it wins on equal :root specificity. The frame
...
const STYLE_ID = "calendar-tokens";
 ...
 */
export function sampleAppliedTokens(names: string[]): Record<string, string> {
