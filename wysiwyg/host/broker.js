/*!
 * wysiwyg/host/broker.js — host-side ADAPTER for the GoFastr WYSIWYG plugin.
 *
 ...
 *
 * This adapter only:
 *   1. registers the wysiwyg manifest (entry URL, sandbox policy, capabilities,
 *      schema) so the generic broker can build the iframe;
 *   2. handles the wysiwyg-SPECIFIC events the generic broker forwards:
 ...
 *      e2e reads (constraint: keep iframe.__wysiwygReady / __wysiwygProbes /
 *      __wysiwygTheme / __wysiwygLastMetric alive):
 *        - ready        → __wysiwygReady + __wysiwygProbes
 *        - themeApplied → __wysiwygTheme
 *        - metric       → __wysiwygLastMetric
 *
 ...
 * wysiwyg.UIHostOption emit pluginhost.js first, then this script.
 */
(function () {
  'use strict';
  ...
  if (!host || typeof host.register !== "function") {
    if (typeof console !== "undefined" && console.error) {
      console.error("[wysiwyg] pluginhost broker not loaded before adapter; mount aborted");
    }
    return;
  }

  // Route constants (mirror the Go plugin consts exactly — protocol-v1.md §2/§10).
  var EDITOR_HTML_URL = "/__gofastr/plugin/wysiwyg/editor.html";
  var SAVE_URL        = "/__gofastr/plugin/wysiwyg/save";
  var UPLOAD_URL      = "/__gofastr/plugin/wysiwyg/upload";
  var SCHEMA_VERSION  = "wysiwyg-v1";
    ...
    onEvent: function (method, params, api) {
      ...
      switch (method) {
        case "ready":
          ...
          frame.__wysiwygReady = true;
          frame.__wysiwygProbes = params.probes || null;
          break;
        case "themeApplied":
          frame.__wysiwygTheme = params; // {scheme, sample:{--name:value}}
          break;
        case "metric":
          frame.__wysiwygLastMetric = params; // {p50, p99, count, …}
          break;
        case "docChanged":
        ...
        default:
          ...
          // generic broker; nothing wysiwyg-specific to do.
          break;
      }
    }
  });
