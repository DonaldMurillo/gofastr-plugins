...
//     window.__gofastrWysiwyg.mountTrusted and the small glue script below is
...
<title>GoFastr WYSIWYG — Trusted Mount Demo</title>
<style>
%s
{{SHELL_CSS}}
.gofastr-wysiwyg-trusted { min-height: 220px; }
</style>
...
<header>
  ...
  <nav aria-label="Demo pages">
    ...
    <a class="navlink is-active" href="/__gofastr/plugin/wysiwyg/trusted" aria-current="page">Trusted</a>
...
<main>
  ...
  <p class="optout-banner"><span aria-hidden="true">⚠️</span><span><strong>This is the explicit opt-out.</strong> The bundle runs with full page access — reserved for plugins the app owner compiles in and vouches for (<code>wysiwyg.WithTrustedMount()</code>, host-side only, never a default).</span></p>
  ...
  <form method="post" action="%s" id="fui-demo-form">
    <section class="editor-card">
      ...
      <div class="gofastr-wysiwyg-trusted">
        <div id="trusted-editor" class="wysiwyg-root"></div>
      </div>
    </section>
...
<footer>
  <p>gofastr-plugins · wysiwyg %s · <a href="/">back to the sandboxed (default) mount →</a></p>
</footer>
<script type="application/json" id="wysiwyg-trusted-doc">%s</script>
<script src="%s"></script>
<script>
(function () {
  ...
  var SAVE_URL = form ? form.getAttribute('action') : '/__gofastr/plugin/wysiwyg/save';
  var UPLOAD_URL = '/__gofastr/plugin/wysiwyg/upload';
  var docEl = document.getElementById('wysiwyg-trusted-doc');
  var initialDoc = null;
  ...
  var api = window.__gofastrWysiwyg.mountTrusted(document.getElementById('trusted-editor'), {
    onEvent: function (method, params) {
      params = params || {};
      switch (method) {
        case 'ready':
          window.__wysiwygTrustedReady = true;
          window.__wysiwygTrustedProbes = params.probes || null;
          break;
        case 'docChanged':
        ...
        case 'save':
          fetch(SAVE_URL, {
            ...
            body: JSON.stringify({ docId: 'demo', doc: params.doc, markdown: params.markdown, schemaVersion: 'wysiwyg-v1' })
          })['catch'](function () {});
          break;
        case 'requestUpload':
          fetch(UPLOAD_URL, {
  ...
  window.__wysiwygTrustedApi = api;
