...
func Render(doc map[string]any) render.HTML {
	...
	return readStyle.WrapHTML(render.Tag("div", map[string]string{"class": "wysiwyg-read"}, blocks...))
}
...
func renderBlock(node map[string]any) (render.HTML, bool) {
	...
	case "task_list":
		return render.Tag("ul", map[string]string{"class": "wysiwyg-task-list"}, renderTaskItems(contentOf(node))...), true
	case "task_item":
		return render.Tag("li", map[string]string{"class": "wysiwyg-task-item"}, renderTaskItem(node)...), true
	case "callout":
	...
	case "column":
		return render.Tag("div", map[string]string{"class": "wysiwyg-column"}, renderBlocks(contentOf(node))...), true
	case "image":
...
func renderCodeBlock(node map[string]any) render.HTML {
	attrs := map[string]string{}
	if lang := strings.TrimSpace(attrString(node, "language")); lang != "" {
		attrs["class"] = "language-" + sanitizeLanguageClass(lang)
	}
	// code_block text carries no marks; concatenate raw text fields, escape once.
	return render.Tag("pre", nil, render.Tag("code", attrs, render.Text(codeBlockText(node))))
}
...
func renderTaskItems(items []any) []render.HTML {
	...
	for _, it := range items {
		...
		// Each item keeps its <li class="wysiwyg-task-item"> wrapper — the
		// checkbox/text row layout is keyed off it (dropping it stacked the
		// checkbox above the text; caught by the dogfood shots).
		out = append(out, render.Tag("li", map[string]string{"class": "wysiwyg-task-item"}, renderTaskItem(node)...))
	}
	return out
}
...
func renderCallout(node map[string]any) render.HTML {
	...
	attrs := map[string]string{
		"class":        "wysiwyg-callout wysiwyg-callout--" + variant,
	...
	if icon := strings.TrimSpace(attrString(node, "icon")); icon != "" {
		...
		iconHTML := render.Tag("span", map[string]string{"class": "wysiwyg-callout__icon", "aria-hidden": "true"}, render.Text(icon))
...
func renderToggle(node map[string]any) render.HTML {
	attrs := map[string]string{"class": "wysiwyg-toggle"}
	...
	for _, c := range contentOf(node) {
		...
		if nodeType(cn) == "toggle_summary" {
			summary = render.Tag("summary", map[string]string{"class": "wysiwyg-toggle__summary"}, renderInline(contentOf(cn)))
			summarySet = true
			continue
		}
	...
	if !summarySet {
		summary = render.Tag("summary", map[string]string{"class": "wysiwyg-toggle__summary"}, render.Text("Details"))
	}
	return render.Tag("details", attrs, summary, render.Tag("div", map[string]string{"class": "wysiwyg-toggle__body"}, body...))
}
...
func renderColumns(node map[string]any) render.HTML {
	...
	for _, c := range contentOf(node) {
		...
		cols = append(cols, render.Tag("div", map[string]string{"class": "wysiwyg-column"}, renderBlocks(contentOf(cn))...))
	}
	attrs := map[string]string{"class": "wysiwyg-columns", "data-count": strconv.Itoa(count)}
...
func renderTable(node map[string]any) render.HTML {
	...
	return render.Tag("table", map[string]string{"class": "wysiwyg-table"}, children...)
}
...
func wrapMark(mark map[string]any, inner render.HTML) render.HTML {
	...
	case "textColor":
		...
		return render.Tag("span", map[string]string{"style": "color:var(--wysiwyg-fg-" + slot + ",var(" + fgFallback[slot] + "))"}, inner)
	case "bgColor":
		slot := normalizeColorSlot(attrString(mark, "color"))
		if slot == "" {
			return inner
		...
		return render.Tag("span", map[string]string{"style": "background-color:var(--wysiwyg-bg-" + slot + ",var(" + bgFallback[slot] + "));color:var(--wysiwyg-bg-ink,#1b1f24)"}, inner)
	}
	return inner
}
