// Build script for the WYSIWYG in-frame editor bundle.
//
...
const banner = {
  js: "/* GoFastr WYSIWYG editor — protocol v1, schemaVersion wysiwyg-v1. Built IIFE; do not edit by hand. */",
};
...
async function main() {
  ...
  // window.__gofastrWysiwyg.mountTrusted instead of auto-booting in a frame.
  const inlineResult = await build({
    ...
    legalComments: "none",
    banner: {
      js: "/* GoFastr WYSIWYG editor — TRUSTED in-page mount (opt-out of the sandbox; DECISIONS.md). Built IIFE; do not edit by hand. */",
    },
    sourcemap: false,
    logLevel: "info",
  });
  ...
  // .gofastr-wysiwyg-trusted (html/body/:root map to the wrapper itself).
  const scopedCSS = scopeCSS(css, ".gofastr-wysiwyg-trusted");
...
const KEEP_WYSIWYG_TOKENS = "__KEEP_WYSIWYG_TOKENS__";
...
function scopeCSS(css, scope) {
  ...
  // Filter the fallback-token block: keep only plugin-local --wysiwyg-*
  // declarations, rescoped under `scope`; drop framework-token fallbacks.
  const lines = out.join("\n").split("\n");
  const kept = [];
  let filtering = false;
  for (const line of lines) {
    if (!filtering && line.includes(KEEP_WYSIWYG_TOKENS)) {
      filtering = true;
      kept.push(scope + " {");
      continue;
    }
    if (filtering) {
      if (line.trim() === "}") {
        filtering = false;
        kept.push("}");
      } else if (line.trim().startsWith("--wysiwyg-")) {
        kept.push(line);
      }
      continue;
    }
...
function scopeSelector(selectorList, scope) {
  return selectorList
    ...
    .map((raw) => {
      ...
      // The PLUGIN-LOCAL --wysiwyg-* slot tokens are different: no host
      ...
      // block down to --wysiwyg-* declarations instead of dropping it.
      if (sel === ":root") return KEEP_WYSIWYG_TOKENS;
