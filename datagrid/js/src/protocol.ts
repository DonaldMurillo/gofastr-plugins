...
//
// Structurally the mermaid/monaco frame client: the protocol is frozen and
// lives in the platform, so every plugin frames the same client. Divergence:
// handler params are `unknown` (never `any`) — each handler narrows its own
// payload defensively — and pending-request timers use a named handle type.
  ...
  return function route(event: MessageEvent): void {
    ...
    if (msg.type === "event") {
      try {
        ...
        console.error("[datagrid] event handler error:", msg.method, err);
      }
      return;
    }
    if (msg.type === "request") {
