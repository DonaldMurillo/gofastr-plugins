...
func (p *Plugin) Init(app *framework.App) error {
	...
	// across plugins). The richtext adapter (host/broker.js) is injected
	...
	// design-token HTML (richtext/ssr). Real content on first paint, no editor
...
// richtext-specific data-fui-plugin-for attribute naming the JSON + markdown
