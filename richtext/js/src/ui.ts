// In-frame UI overlays for the Rich Text editor (schema richtext-v1).
//
...
const slashKey = new PluginKey("richtext-slash");
const bubbleKey = new PluginKey("richtext-bubble");
const handleKey = new PluginKey("richtext-draghandle");
const toolbarKey = new PluginKey("richtext-toolbar");
const statusKey = new PluginKey("richtext-status");
const blockCtlKey = new PluginKey("richtext-blockctl");
...
function icon(svg: string) {
  return el("span", { class: "richtext-ico", html: svg });
}
...
// inside the `.gofastr-richtext-trusted` CSS scope (they are position:fixed, so
...
class SlashMenu {
  ...
  render() {
    ...
    root.innerHTML = "";
    if (!items.length) {
      root.appendChild(el("div", { class: "richtext-slash-empty" }, "No matches"));
    }
    items.forEach((it, i) => {
      const opt = el(
        "div",
        {
          ...
          class: "richtext-slash-item" + (i === this.selected ? " is-active" : ""),
          role: "option",
          "aria-selected": i === this.selected ? "true" : "false",
        },
        el("div", { class: "richtext-slash-title" }, it.title),
        it.sub ? el("div", { class: "richtext-slash-sub" }, it.sub) : null
      );
  ...
  updateActive() {
    const opts = this.root!.querySelectorAll(".richtext-slash-item");
    opts.forEach((o, i) => {
...
const SLASH_LISTBOX_ID = "richtext-slash-listbox";
const SLASH_OPT_PREFIX = "richtext-slash-opt-";
...
function markButton(
  ...
  const btn = el(
    ...
    {
      type: "button",
      class: "richtext-tbtn" + (active ? " is-active" : ""),
      title: label,
      "aria-label": label,
...
class Bubble {
  ...
  render() {
    ...
    root.appendChild(el("span", { class: "richtext-tbtn-sep" }));
    root.appendChild(
    ...
    const colorBtn = el(
      "button",
      { type: "button", class: "richtext-tbtn" + (textOpen ? " is-active" : ""), title: "Text color", "aria-label": "Text color", "aria-haspopup": "listbox", "aria-expanded": textOpen ? "true" : "false" },
      icon(ICON.text)
    );
    colorBtn.appendChild(el("span", { class: "richtext-color-bar" }));
    ...
    const hlBtn = el(
      "button",
      { type: "button", class: "richtext-tbtn" + (bgOpen ? " is-active" : ""), title: "Highlight", "aria-label": "Background highlight", "aria-haspopup": "listbox", "aria-expanded": bgOpen ? "true" : "false" },
      icon(ICON.highlight)
    );
  ...
  renderColorPicker() {
    const root = this.root!;
    root.querySelectorAll(".richtext-colorgrid").forEach((n) => n.remove());
  ...
  openLinkEditor() {
    this.render();
    const root = this.root!;
    root.querySelectorAll(".richtext-linkpop").forEach((n) => n.remove());
    const v = this.view;
    const existing = cmd.activeLinkMark(v.state);
    const wrap = el("div", { class: "richtext-linkpop", role: "dialog", "aria-label": "Edit link" });
    const hrefInput = el("input", {
      type: "url",
      class: "richtext-link-input",
      placeholder: "https://…",
      value: existing ? existing.attrs.href : "",
      "aria-label": "Link URL",
    });
    const applyBtn = el("button", { type: "button", class: "richtext-link-apply" }, existing ? "Update" : "Apply");
    const removeBtn = el("button", { type: "button", class: "richtext-link-remove" }, "Remove");
    // Inline error/feedback line (no more silent-close on a bad scheme).
    const err = el("div", { class: "richtext-link-error", role: "alert", style: { display: "none" } });
    const doApply = () => {
...
class DragHandle {
  ...
  build() {
    this.handle = el(
      ...
      el("span", { class: "richtext-draghandle-grip", "aria-hidden": "true" })
    );
    ...
    this.indicator = el("div", { class: "richtext-drop-indicator", style: { display: "none" } });
...
class Toolbar {
  ...
  build() {
    const v = this.view;
    const bar = el("div", { class: "richtext-toolbar", role: "toolbar", "aria-label": "Formatting" });
    this.root = bar;
    this.bar = bar;

    const group = (...kids: (HTMLElement | null)[]) => {
      const g = el("div", { class: "richtext-tb-group" });
      kids.forEach((k) => k && g.appendChild(k));
      return g;
    };
    const sep = () => el("span", { class: "richtext-tbtn-sep" });
    ...
    // Block-type selector.
    const select = el("select", { class: "richtext-tb-select", "aria-label": "Block type" }) as HTMLSelectElement;
    ...
    const wrap = el("div", { class: "richtext-toolbar-wrap" });
  ...
  tbtn(label: string, ic: string, onPress: () => void) {
    const b = el(
      "button",
      { type: "button", class: "richtext-tbtn", title: label, "aria-label": label, "aria-pressed": "false" },
      icon(ic)
    );
    activate(b, onPress);
    return b;
  ...
  colorButton(kind: "text" | "bg") {
    ...
    const b = this.tbtn(label, kind === "bg" ? ICON.highlight : ICON.text, () => {
      ...
      this.closeColor();
      this.colorOpen = kind;
      const pop = buildColorGrid(this.view, kind, () => this.closeColor());
      // Float it over everything via place() — appending it INSIDE the toolbar
      // group clipped it against the toolbar's horizontal-scroll overflow.
      pop.classList.add("richtext-colorgrid-float");
      this.colorPop = pop;
      b.setAttribute("aria-expanded", "true");
      overlayParent().appendChild(pop);
      const r = b.getBoundingClientRect();
      place(pop, { left: r.left + r.width / 2, top: r.top, bottom: r.bottom }, "below", 6);
      // Dismiss on outside pointerdown (mirrors the link popover).
...
function buildColorGrid(view: EditorView, kind: "text" | "bg", onDone: () => void): HTMLElement {
  const grid = el("div", {
    class: "richtext-colorgrid richtext-colorgrid-" + kind,
    role: "listbox",
  ...
  COLOR_SLOTS.forEach((slot) => {
    const sw = el("button", {
      type: "button",
      class: "richtext-swatch richtext-swatch-" + slot,
      role: "option",
      title: slot,
      "aria-label": slot,
      "data-slot": slot,
    });
    sw.style.color = `var(--richtext-fg-${slot}, var(--color-text, currentColor))`;
    sw.style.background = `var(--richtext-bg-${slot}, var(--color-surface, transparent))`;
    activate(sw, () => {
  ...
  const clear = el("button", { type: "button", class: "richtext-swatch richtext-swatch-clear", title: "Clear color", "aria-label": "Clear color" }, "×");
  activate(clear, () => {
...
function openLinkPopover(view: EditorView, anchor: HTMLElement, existing: boolean) {
  document.querySelectorAll(".richtext-linkpop-float").forEach((n) => n.remove());
  const wrap = el("div", { class: "richtext-linkpop richtext-linkpop-float", role: "dialog", "aria-label": "Edit link" });
  ...
  wrap.appendChild(applyBtn);
  if (existing) {
    const rm = el("button", { type: "button", class: "richtext-link-remove" }, "Remove");
    activate(rm, () => { run(view, cmd.unsetLink()); close(); });
    const open = el("a", { class: "richtext-link-open", href: cur ? cur.attrs.href : "#", target: "_blank", rel: "noopener noreferrer", title: "Open link", "aria-label": "Open link in a new tab" }, "↗");
...
class BlockControls {
  ...
  constructor(view: EditorView) {
    this.view = view;
    this.bar = el("div", { class: "richtext-blockctl", style: { display: "none" } });
  ...
  tableControls(): HTMLElement {
    const v = this.view;
    const g = el("div", { class: "richtext-blockctl-group", role: "toolbar", "aria-label": "Table" });
    const btn = (label: string, c: () => Command) => {
      const b = el("button", { type: "button", class: "richtext-blockctl-btn", title: label, "aria-label": label }, label);
      activate(b, () => run(v, c()));
      return b;
    };
  ...
  codeControls(pos: number): HTMLElement {
    ...
    const g = el("div", { class: "richtext-blockctl-group" });
    const sel = el("select", { class: "richtext-blockctl-select", "aria-label": "Code language" }) as HTMLSelectElement;
...
class StatusBar {
  ...
  constructor(view: EditorView) {
    this.view = view;
    this.root = el("div", { class: "richtext-statusbar", role: "status", "aria-live": "off" });
