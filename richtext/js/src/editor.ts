// GoFastr Rich Text editor — in-frame entry point (protocol v1).
//
...
//
// Full block set (schema richtext-v1): every node/mark in docs/design/schema-v1.md.
...
function stateFromInit(doc: unknown, markdown: unknown): EditorState {
  ...
  if (doc && typeof doc === "object") {
    try {
      ...
      console.warn("[richtext] init.doc rejected by schema, trying markdown:", err);
      pmDoc = null;
    }
  }
...
function onDecorationKeydown(event: KeyboardEvent) {
  ...
  const checkbox = target.closest(".richtext-task-checkbox");
  const summary = target.closest(".richtext-toggle-summary");
...
function requestImageUpload(file: File, pos?: number | null) {
  ...
  file
    ...
    .catch((err) => console.warn("[richtext] upload read failed:", err));
}
...
function handleInit(params: Record<string, unknown>) {
  ...
  if (p.schemaVersion && p.schemaVersion !== SCHEMA_VERSION) {
    console.warn(
      `[richtext] schemaVersion mismatch: host=${p.schemaVersion} frame=${SCHEMA_VERSION}`
    );
  }
...
function overlayExtent(): number {
  let max = 0;
  const sels = [".richtext-slash-menu", ".richtext-bubble", ".richtext-linkpop"];
...
export function bootFrame() {
  try {
    ...
    if (!root) {
      console.error("[richtext] mount root", ROOT_SELECTOR, "not found");
      return;
    }
...
// explicit host-side opt-in (richtext.WithTrustedMount on the Go side).
...
export function mountTrusted(el: Element, opts: TrustedMountOptions = {}): TrustedMountHandle {
  ...
  if (trustedMounted) throw new Error("richtext: an editor is already mounted on this page");
  trustedMounted = true;
  root = el;
