...
func newTestApp(t *testing.T, opts ...Option) (*framework.App, *Plugin) {
	t.Helper()
	p := New(opts...)
	app := framework.NewApp(framework.WithConfig(framework.AppConfig{Name: "richtext-test"}))
...
// serving richtext's framed assets) carries the framing/CORP/CSP relaxation
...
func TestSaveRoundTripDevGrant(t *testing.T) {
	...
	payload := `{"docId":"demo","doc":` + doc + `,"markdown":"hi","schemaVersion":"richtext-v1"}`
...
func TestSaveDeniedWithoutCapability(t *testing.T) {
	...
	payload := `{"docId":"demo","doc":null,"markdown":"","schemaVersion":"richtext-v1"}`
	deniedCtx := auth.WithTokenScopes(context.Background(), []string{"posts:read"})
