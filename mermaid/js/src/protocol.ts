...
// Handler params are intentionally `any`: postMessage payloads are untyped at
// the boundary; each handler validates/narrows its own params defensively.
type Handler = (params: any, msg: ProtocolMessage) => unknown;
  ...
  return function route(event: MessageEvent): void {
    ...
    if (msg.type === "event") {
      try {
        ...
        console.error("[mermaid] event handler error:", msg.method, err);
      }
      return;
    }
    if (msg.type === "request") {
